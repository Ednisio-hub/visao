...
app.post("/detectarLabels", upload.single("myfile"), function(
  request,
  response
) {
...
app.post("/detectarModeracao", upload.single("myfile_moderar"), function(
  request,
  response
) {
  var arquivo = request.file.buffer;
...
app.post("/analiseFacial", upload.single("myfile_facial"), function(
  request,
  response
) {
  var arquivo = request.file.buffer;
...
app.post("/reconherCelebridade", upload.single("myfile_celebridade"), function(
  request,
  response
) {
  var arquivo = request.file.buffer;
...
app.post("/reconherTexto", upload.single("myfile_texto"), function(
  request,
  response
) {
  var arquivo = request.file.buffer;
...
app.post("/indexar", upload.single("myfile_index"), function(
  request,
  response
) {
  var arquivo = request.file.buffer;
...
app.post("/procurar", upload.single("myfile_procurar"), function(
  request,
  response
) {
  var arquivo = request.file.buffer;
  ...
  rekognition.getLabelDetection(params, function(err, data) {
    if (err) {
      ...
      for (var i = 0; i < data.Labels.length; i++) {
        ...
        table +=
          "<td>" +
          parseFloat(data.Labels[i].Label.Confidence).toFixed(2) +
          "</td>";
        table +=
          "<td>" +
          moment
            .duration(data.Labels[i].Timestamp, "milliseconds")
  ...
  rekognition.getCelebrityRecognition(params, function(err, data) {
    if (err) {
      ...
      for (var i = 0; i < data.Celebrities.length; i++) {
        ...
        table +=
          "<td>" +
          parseFloat(data.Celebrities[i].Celebrity.Confidence).toFixed(2) +
          "</td>";
        table +=
          "<td><a href=https://" +
          data.Celebrities[i].Celebrity.Urls +
          ">Clique Aqui </a></td>";
        table +=
          "<td>" +
          moment
            .duration(data.Celebrities[i].Timestamp, "milliseconds")
            .format("mm:ss:SS", { trim: false });
        +"</td>";
        table += "</tr>";
      }
